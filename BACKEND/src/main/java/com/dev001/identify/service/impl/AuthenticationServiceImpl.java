...
public class AuthenticationServiceImpl implements AuthenticationService {
    ...
    //        );
    //        // 2. Check user exist
    //        User user = userRepository.findByUsername(request.getUsername()).orElseThrow(() -> new
    ...
    // AppException(REFRESH_TOKEN_EXPIRED));
    //        // 5. check if token is expired or not valid throw ex
    //        if(!jwtService.isTokenValid(refreshToken, userDetails)) {
    //            throw new AppException(REFRESH_TOKEN_EXPIRED);
    //        }
    //        // 6. check if token is revoked or not throw ex
    //        var isTokenRevoked = tokenRepository.findByToken(refreshToken).map(token ->
    ...
    public void refreshToken(HttpServletRequest request, HttpServletResponse response) {
        ...
        // 4. check if user not exist in db throw ex
        var userDetails =
                userRepository.findByUsername(userName).orElseThrow(() -> new AppException(REFRESH_TOKEN_EXPIRED));
        // 5. check if token is expired or not valid throw ex
        if (!jwtService.isTokenValid(refreshToken, userDetails)) {
            throw new AppException(REFRESH_TOKEN_EXPIRED);
        }
